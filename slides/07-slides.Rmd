...
.box-inv-6[Create a grid of values you want to<br>manipulate and values you want to hold constant]

--

.box-6[Use `datagrid()` from `marginaleffects` — no need to build the tibble manually]
...
cars_new_data <- datagrid(model = car_model_big,
                          displ = seq(2, 7, by = 0.1),
                          cyl   = mean(mpg$cyl),
                          drv   = "f")
...
.box-inv-6[Plug each row into the model with `predictions()`]

.small-code[
```{r predict-simple-cars-data}
predicted_mpg <- predictions(car_model_big, newdata = cars_new_data)
...
.box-6[Pass multiple values to `datagrid()` — it creates all combinations automatically]
...
cars_new_data_fancy <- datagrid(model = car_model_big,
                                displ = seq(2, 7, by = 0.1),
                                cyl   = mean(mpg$cyl),
                                drv   = c("f", "r", "4"))
...
.small-code[
```{r predict-complex-cars-data}
predicted_mpg_fancy <- predictions(car_model_big, newdata = cars_new_data_fancy)
