...
.box-4.small[Odds > 1 → event more likely to happen]

.box-4.small[Odds < 1 → event less likely to happen]

.sp-after[&nbsp;]

Odds are **always positive** — still not quite what we need
]
...
Taking the **logarithm** of odds gives us something that can be negative, zero, or positive:
...
If Odds = 3 → Log Odds = $\ln(3) \approx 1.10$

If Odds = 1 → Log Odds = $\ln(1) = 0$

If Odds < 1 → Log Odds is **negative**
]
...
This makes them compatible with linear regression
]
...
.pull-right[
```{r logit-plot, echo=FALSE, fig.height=4.5, fig.width=5.5}
library(ggplot2)
library(tibble)
...
The player is 3× more likely to make it than miss
...
.box-4.small[**Probability:** 75% chance of success<br>*(intuitive, limited to 0–1)*]

.box-4.small[**Odds:** 3 to 1 in favour<br>*(useful for comparing success vs. failure)*]

.box-4.small[**Log odds:** 1.10<br>*(mathematical, usable in regression)*]

.box-inv-4.sp-before[Logistic regression works on the<br>log-odds scale, then converts back<br>to probability for interpretation]
]
...
For a binary outcome $y$ and a predictor $x$:

$$\text{logit}(p) = \beta_0 + \beta_1 x$$

--

Because the logit is **invertible**, we can also write this as:
...
.pull-left[
.box-inv-4[This is the S-shaped logistic curve]

.box-inv-4[It is always bounded between 0 and 1]

.box-inv-4[$\beta_0$ and $\beta_1$ are estimated from data using `glm()`]
]

.pull-right[
.box-4[**Admissions example**]

$$\beta_0 = -12.04, \quad \beta_1 = 4.08$$

$$\text{logit}(p) = -12.04 + 4.08 \times \text{GPA}$$

Each 1-point GPA increase multiplies odds by $e^{4.08} \approx 59$
]
...
.box-4.small[`"logit"` link is the default for binomial]
]

.pull-right[
.box-inv-4.small[See the practical session for]
.box-inv-4.small[model fitting, evaluation,]
.box-inv-4.small[confusion matrices, and]
.box-inv-4.small[small multiples of predictions]
]
