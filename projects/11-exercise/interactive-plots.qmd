...
- Don't try to make this interactive right away! [Just like in the example](https://timmarchand.github.io/iss_dataviz/example/11-example.html){target="_blank"}, first make a regular static version before feeding it into `ggplotly()`.
...
- The interactive tooltip should be structured like this:
  ...
  You'll need to make a new column in the penguins dataset named `label` (or whatever you want to call it) that builds that label using that template. [The example will be useful](https://timmarchand.github.io/iss_dataviz/example/11-example.html#including-more-information-in-the-tooltip){target="_blank"}.
